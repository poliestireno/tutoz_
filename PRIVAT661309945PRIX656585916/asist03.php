...
if(!existeFantasma($db,$_SESSION["idAsignatura"],$_SESSION["dia"])){
  ...
  //var_dump($_POST);
  //var_dump($_SESSION);
  borrarFaltasAsignaturaDiaAlumno($db,$_SESSION["idAsignatura"],$_SESSION["dia"],$_POST['alumnoSel']);
  if ($_POST["nFaltasInsertar"]>0)
  {
    insertarFalta($db,$_POST['alumnoSel'],$_SESSION["idAsignatura"],$_POST["nFaltasInsertar"],$_SESSION["dia"]);
  }
  sort($alumnos1);
...
{
  <?php
  if (isset($_POST["tipoMas"]) && ($_POST["tipoMas"]=="-/+"))
  {
    echo 'document.getElementById("idMas").innerHTML="-/+"';
  }
  else
  {
    echo 'document.getElementById("idMas").innerHTML="+/-"';
...
function managebutton(faltas,alSel)
{
...
for($i = 0; $i < $arrlength+2; $i++) 
{
  ...
  if ($i==$arrlength)
  {
    echo '<a id="idMas" btn-outline btn-wrap-text style="font-size:'.($sizeT/2.5).'px; height: '.$sizeT.'px" onclick="managebuttonMas()" class="btn btn-primary">+/-</a>';      
  }
  else if ($i==$arrlength+1)
  {
    ...
    echo '<a id="bb_'.$i.'" btn-outline btn-wrap-text style="font-size:'.($sizeT/2.5).'px; height: '.$sizeT.'px" onclick="managebuttonBack()" class="btn btn-dark"> '.substr($_SESSION["dia"],5,strlen($_SESSION["dia"])).",".$_SESSION["nSesiones"].",".$clase.'('.$nTotalAsistencia.') </a>';      
  }
  else
  {
    ...
    $nFaltas = getFaltasFromAlumnoDiaAsignatura($db, $idAlumnoo, $_SESSION["dia"], $_SESSION["idAsignatura"]);
    $fantasma = getFantasma($db,$_SESSION["idAsignatura"],$_SESSION["dia"]);
    $contElegTotal= $fantasma["ELEGIDOS"];
    $datos = explode(',', $contElegTotal);
    $colorBoton=(($nFaltas==0)?'btn btn-success':'btn btn-danger');
    if ($nFaltas==0)
    {
      $nTotalAsistencia=$nTotalAsistencia+1;
    }
    foreach ($datos as $elegido)
    {
      if ($elegido==$idAlumnoo)
      {
        $colorBoton = "btn btn-warning";
      }
    }
    echo '<a id="bb_'.$i.'" style="font-size:'.($sizeT/2.5).'px; height: '.(floor($_POST["altura"]/$filas)).'px" onclick="managebutton('.$nFaltas.','.$idAlumnoo.')" class="'.$colorBoton.'">'.$nFaltas.'-'.$alumnoaux.'</a>';
  }
  $cont++;
  if ($cont % $limite == 0)
  {
