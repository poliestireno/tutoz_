...
let gameState = "level"+(Math.floor(Math.random() * 3) + 1);
...
function displayGameOver() {
  if (gameState == "gameover") {
    ...
    setTimeout(displayRestartGame, 1500);
  }
}
...
function displayLevel() {
  ...
  if (gameState == "level4") {
    ...
    body.style.backgroundImage =
      "radial-gradient(circle, white 0%, #FD746C 50%,  #2C3E50 100%)";
    displayLevelName();
  }
  if (gameState == "level5") {
...
function ballCollisionsPaddle() {
  if (
    ball.y + (ball.radius - 1) > paddle.y &&
    ball.y + (ball.radius - 1) < paddle.y + paddle.height &&
    ball.x > paddle.x &&
...
function ballCollisionsBricks() {
  for (r = 0; r < brick.rows; r++) {
    for (c = 0; c < brick.columns; c++) {
      //nested for loops needed to have access to the bricks[][] again.

      let b1 = bricks[0][c];
      if (b1.unbroken) {
        if (
          ball.x + ball.radius > b1.x &&
          ball.x - ball.radius < b1.x + brick.width &&
          ball.y + ball.radius > b1.y &&
          ball.y - ball.radius < b1.y + brick.height
        ) {
          soundBricks.play();
          b1.unbroken = false;
          ball.ySpeed = -ball.ySpeed;
      ...
      if (b2.unbroken) {
        if (
          ball.x + ball.radius > b2.x &&
          ball.x - ball.radius < b2.x + brick.width &&
          ball.y + ball.radius > b2.y &&
          ball.y - ball.radius < b2.y + brick.height
        ) {
          soundBricks.play();
          b2.unbroken = false;
          ball.ySpeed = -ball.ySpeed;
      ...
      if (b3.unbroken) {
        if (
          ball.x + ball.radius > b3.x &&
          ball.x - ball.radius < b3.x + brick.width &&
          ball.y + ball.radius > b3.y &&
          ball.y - ball.radius < b3.y + brick.height
        ) {
          soundBricks.play();
          b3.unbroken = false;
          ball.ySpeed = -ball.ySpeed;
      ...
      if (brick.rows > 3) {
        let b4 = bricks[3][c];
        if (
          (gameState === "playL2" && b4.unbroken) ||
          (gameState === "playL3" && b4.unbroken) ||
          (gameState === "playL4" && b4.unbroken) ||
          (gameState === "playL5" && b4.unbroken)
        ) {
          if (
            ball.x + ball.radius > b4.x &&
            ball.x - ball.radius < b4.x + brick.width &&
            ball.y + ball.radius > b4.y &&
            ball.y - ball.radius < b4.y + brick.height
          ) {
            soundBricks.play();
            b4.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
      ...
      if (brick.rows > 4) {
        let b5 = bricks[4][c];
        if (
          (gameState === "playL2" && b5.unbroken) ||
          (gameState === "playL3" && b5.unbroken) ||
          (gameState === "playL4" && b5.unbroken) ||
          (gameState === "playL5" && b5.unbroken)
        ) {
          if (
            ball.x + ball.radius > b5.x &&
            ball.x - ball.radius < b5.x + brick.width &&
            ball.y + ball.radius > b5.y &&
            ball.y - ball.radius < b5.y + brick.height
          ) {
            soundBricks.play();
            b5.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
      ...
      if (brick.rows > 5) {
        let b6 = bricks[5][c];
        if (
          (gameState === "playL2" && b6.unbroken) ||
          (gameState === "playL3" && b6.unbroken) ||
          (gameState === "playL4" && b6.unbroken) ||
          (gameState === "playL5" && b6.unbroken)
        ) {
          if (
            ball.x + ball.radius > b6.x &&
            ball.x - ball.radius < b6.x + brick.width &&
            ball.y + ball.radius > b6.y &&
            ball.y - ball.radius < b6.y + brick.height
          ) {
            soundBricks.play();
            b6.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
      ...
      if (brick.rows > 6) {
        let b7 = bricks[6][c];
        if (
          (gameState === "playL3" && b7.unbroken) ||
          (gameState === "playL4" && b7.unbroken) ||
          (gameState === "playL5" && b7.unbroken)
        ) {
          if (
            ball.x + ball.radius > b7.x &&
            ball.x - ball.radius < b7.x + brick.width &&
            ball.y + ball.radius > b7.y &&
            ball.y - ball.radius < b7.y + brick.height
          ) {
            soundBricks.play();
            b7.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b8 = bricks[7][c];
        if (
          (gameState === "playL3" && b8.unbroken) ||
          (gameState === "playL4" && b8.unbroken) ||
          (gameState === "playL5" && b8.unbroken)
        ) {
          if (
            ball.x + ball.radius > b8.x &&
            ball.x - ball.radius < b8.x + brick.width &&
            ball.y + ball.radius > b8.y &&
            ball.y - ball.radius < b8.y + brick.height
          ) {
            soundBricks.play();
            b8.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b9 = bricks[8][c];
        if (
          (gameState === "playL3" && b9.unbroken) ||
          (gameState === "playL4" && b9.unbroken) ||
          (gameState === "playL5" && b9.unbroken)
        ) {
          if (
            ball.x + ball.radius > b9.x &&
            ball.x - ball.radius < b9.x + brick.width &&
            ball.y + ball.radius > b9.y &&
            ball.y - ball.radius < b9.y + brick.height
          ) {
            soundBricks.play();
            b9.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
      ...
      if (brick.rows > 9) {
        let b10 = bricks[9][c];
        if (
          (gameState === "playL4" && b10.unbroken) ||
          (gameState === "playL5" && b10.unbroken)
        ) {
          if (
            ball.x + ball.radius > b10.x &&
            ball.x - ball.radius < b10.x + brick.width &&
            ball.y + ball.radius > b10.y &&
            ball.y - ball.radius < b10.y + brick.height
          ) {
            soundBricks.play();
            b10.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b11 = bricks[10][c];
        if (
          (gameState === "playL4" && b11.unbroken) ||
          (gameState === "playL5" && b11.unbroken)
        ) {
          if (
            ball.x + ball.radius > b11.x &&
            ball.x - ball.radius < b11.x + brick.width &&
            ball.y + ball.radius > b11.y &&
            ball.y - ball.radius < b11.y + brick.height
          ) {
            soundBricks.play();
            b11.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b12 = bricks[11][c];
        if (
          (gameState === "playL4" && b12.unbroken) ||
          (gameState === "playL5" && b12.unbroken)
        ) {
          if (
            ball.x + ball.radius > b12.x &&
            ball.x - ball.radius < b12.x + brick.width &&
            ball.y + ball.radius > b12.y &&
            ball.y - ball.radius < b12.y + brick.height
          ) {
            soundBricks.play();
            b12.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b13 = bricks[12][c];
        if (
          (gameState === "playL4" && b13.unbroken) ||
          (gameState === "playL5" && b13.unbroken)
        ) {
          if (
            ball.x + ball.radius > b13.x &&
            ball.x - ball.radius < b13.x + brick.width &&
            ball.y + ball.radius > b13.y &&
            ball.y - ball.radius < b13.y + brick.height
          ) {
            soundBricks.play();
            b13.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b14 = bricks[13][c];
        if (
          (gameState === "playL4" && b14.unbroken) ||
          (gameState === "playL5" && b14.unbroken)
        ) {
          if (
            ball.x + ball.radius > b14.x &&
            ball.x - ball.radius < b14.x + brick.width &&
            ball.y + ball.radius > b14.y &&
            ball.y - ball.radius < b14.y + brick.height
          ) {
            soundBricks.play();
            b14.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b15 = bricks[14][c];
        if (
          (gameState === "playL4" && b15.unbroken) ||
          (gameState === "playL5" && b15.unbroken)
        ) {
          if (
            ball.x + ball.radius > b15.x &&
            ball.x - ball.radius < b15.x + brick.width &&
            ball.y + ball.radius > b15.y &&
            ball.y - ball.radius < b15.y + brick.height
          ) {
            soundBricks.play();
            b15.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b16 = bricks[15][c];
        if (
          (gameState === "playL4" && b16.unbroken) ||
          (gameState === "playL5" && b16.unbroken)
        ) {
          if (
            ball.x + ball.radius > b16.x &&
            ball.x - ball.radius < b16.x + brick.width &&
            ball.y + ball.radius > b16.y &&
            ball.y - ball.radius < b16.y + brick.height
          ) {
            soundBricks.play();
            b16.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b17 = bricks[16][c];
        if (
          (gameState === "playL4" && b17.unbroken) ||
          (gameState === "playL5" && b17.unbroken)
        ) {
          if (
            ball.x + ball.radius > b17.x &&
            ball.x - ball.radius < b17.x + brick.width &&
            ball.y + ball.radius > b17.y &&
            ball.y - ball.radius < b17.y + brick.height
          ) {
            soundBricks.play();
            b17.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b18 = bricks[17][c];
        if (
          (gameState === "playL4" && b18.unbroken) ||
          (gameState === "playL5" && b18.unbroken)
        ) {
          if (
            ball.x + ball.radius > b18.x &&
            ball.x - ball.radius < b18.x + brick.width &&
            ball.y + ball.radius > b18.y &&
            ball.y - ball.radius < b18.y + brick.height
          ) {
            soundBricks.play();
            b18.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
        ...
        let b19 = bricks[18][c];
        if (
          (gameState === "playL4" && b19.unbroken) ||
          (gameState === "playL5" && b19.unbroken)
        ) {
          if (
            ball.x + ball.radius > b19.x &&
            ball.x - ball.radius < b19.x + brick.width &&
            ball.y + ball.radius > b19.y &&
            ball.y - ball.radius < b19.y + brick.height
          ) {
            soundBricks.play();
            b19.unbroken = false;
            ball.ySpeed = -ball.ySpeed;
