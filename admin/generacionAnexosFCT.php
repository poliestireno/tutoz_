...
?>
  <center><img id="imgProcc" src="img/procc.gif"/></center>
<?php
...
foreach($_POST as $key => $value)
		{	    
				...
				{
							$numAlum = substr($key,4,strlen($key));
							$aNumAlumnos[]=$numAlum;
				}
...
<body onload="document.getElementById('imgProcc').parentNode.removeChild(document.getElementById('imgProcc'));">
...
function generarAnexo7($folder, $sTextoCarpetaAMostrar,$num,$aNombres,$aNombresConApellido1,$aDNIs,$idFolder,$semanas)
{
$token = strtok($semanas, ",");
$aSemanas=array();
while ($token !== false)
{
	if (trim($token)!="")
...
$numSemanas = Count($aSemanas);

$rutaPlantillaCiclo = "../PHPWord-develop/plantillas/anexos7/anexo7_".$numSemanas.".docx";
if (file_exists($rutaPlantillaCiclo)) 
{
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor($rutaPlantillaCiclo);
}
...
{
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../PHPWord-develop/plantillas/anexos7/anexo7_15.docx');
}

// todas las semanas
for ($i=0; $i < $numSemanas; $i++) 
{ 
	$templateProcessor->setValue('DATOS_SEMANA'.($i+1), $aSemanas[$i]);
}
//Empresa
$templateProcessor->setValue('NOMBRE_EMPRESA', mb_strtoupper($_POST['NOMBRE_EMPRESA'.$num], 'UTF-8'));
...
{
	  global $aNumAlumnos;
	  $empresaAnterior=$_POST['NOMBRE_EMPRESA'.$aNumAlumnos[0]];
    $aNombres = array();
    ...
    $aDNIs = array();
  	...
  	for ($k=0; $k < Count($aNumAlumnos); $k++) 
		{ 
				...
				{	
					echo '<a class="list-group-item ">'.'&emsp;&emsp;'.generarAnexo21($folder,$sTextoCarpetaAMostrar,$aNumAlumnos[$k-1],$aNombres,$aNombresConApellido1,$aDNIs,$idFolder).'</a>';	
    ...
    $aDNIs = array();
  	...
  	for ($k=0; $k < Count($aNumAlumnos); $k++) 
		{ 
			...
			{
				echo '<a class="list-group-item ">'.'&emsp;&emsp;'.generarAnexo3($folder,$sTextoCarpetaAMostrar,$aNumAlumnos[$k-1],$aNombres,$aNombresConApellido1,$aDNIs,$idFolder).'</a>';
				$empresaAnterior = $_POST['NOMBRE_EMPRESA'.$i];
				$aNombres = array();
				$aNombresConApellido1 = array();
    		$aDNIs = array();
...
function bloqueAnexo7($sTextoCarpetaAMostrar,$folder,$idFolder,$semanas)
{
	  global $aNumAlumnos;
    $aNombres = array();
    $aNombresConApellido1 = array();   
